...
\ (GPIO ACC  -- GPIO ACC )
: ZERONEREAD SWAP 2DUP 29 DELAY READLEV DUP 0 = IF DATI ROT ASTORE  ELSE DATI ROT ASTORE BEGIN DUP READLEV 1 = WHILE REPEAT THEN  SWAP  ;

\LETTURA DATI GPLEV
\( STANZA GPIO -- STANZA GPIO)
\: READS INITACC INITTI BEGIN ACC @ N_BITS < TIMESTAMP TI @ - ALT < AND  WHILE BEGIN DUP READLEV  0 = WHILE REPEAT ACC @  ZERONEREAD 1 SWAP ADDC REPEAT  ACC @ . ;
\: READS INITTI BEGIN TIMESTAMP TI @ - ALT < WHILE DUP READLEV SWAP REPEAT ;
